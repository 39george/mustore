...
#[allow(non_camel_case_types)] pub enum Userrole { creator,consumer,fullstack,}impl < 'a > postgres_types :: ToSql for Userrole
{
    fn
    ...
    {
        let s = match * self { Userrole :: creator => "creator",Userrole :: consumer => "consumer",Userrole :: fullstack => "fullstack",}
        ; buf.extend_from_slice(s.as_bytes()) ; std :: result :: Result ::
        Ok(postgres_types :: IsNull :: No)
    } fn accepts(ty : & postgres_types :: Type) -> bool
    {
        if ty.name() != "userrole" { return false ; } match * ty.kind()
        {
            postgres_types :: Kind :: Enum(ref variants) =>
            {
                if variants.len() != 3 { return false ; }
                variants.iter().all(| v | match & * * v
                { "creator" => true,"consumer" => true,"fullstack" => true,_ => false, })
            } _ => false,
        }
    } fn
...
} impl < 'a > postgres_types :: FromSql < 'a > for Userrole
{
    ...
    Userrole, Box < dyn std :: error :: Error + Sync + Send >, >
    {
        match std :: str :: from_utf8(buf) ?
        {
            "creator" => Ok(Userrole :: creator),"consumer" => Ok(Userrole :: consumer),"fullstack" => Ok(Userrole :: fullstack),s => Result ::
...
}#[derive( Debug, Clone, PartialEq, )] pub struct GetAuthUserDataById
...
VALUES ($1, $2, NULL, $3, $4, NULL, $5) returning id")) } pub
...
(& 'a mut self, client : & 'a  C,
user_settings_id : & 'a i32,username : & 'a T1,email : & 'a T2,password_hash : & 'a T3,role : & 'a super::super::types::public::Userrole,) -> I32Query < 'a, C,
i32, 5 >
{
    I32Query
    {
        client, params : [user_settings_id,username,email,password_hash,role,], stmt : & mut self.0, extractor :
        | row | { row.get(0) }, mapper : | it | { it },
    }
...
C, i32, 5 >, C > for InsertNewUserStmt
{
    fn
    params(& 'a mut self, client : & 'a  C, params : & 'a
    InsertNewUserParams < T1,T2,T3,>) -> I32Query < 'a, C,
    i32, 5 >
    { self.bind(client, & params.user_settings_id,& params.username,& params.email,& params.password_hash,& params.role,) }

...
impl EmailClient {
    pub fn new(
        ...
        let base_url = reqwest::Url::try_from(base_url.as_str())?;
    ...
    /// email delivery service, with data necessary
    /// to send the email to a recipient.
...
mod email_client_tests {
    ...
    fn email() -> UserEmail {
        UserEmail::parse(&SafeEmail().fake::<String>()).unwrap()
    }
